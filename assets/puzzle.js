...
// canv for the moving PolyPiece and the full image
  ...
  if (!this.menu) {
    this.menu = new Menu({
      ...
      lineStep: 30,
      lines: [
        {text: "Custom image", func: this.loadImage()},
...
Puzzle.prototype.associateImage = function() {
  ...
  // Updated conditions for centering the puzzle pieces.
  if (wi / he > this.width / this.height) {
    scale = this.height / he;
    offsx = (this.width - wi * scale) / 2;
    offsy = 0;
  } else {
    scale = this.width / wi;
    offsy = (this.height - he * scale) / 2;
    offsx = 0;
  }
  ...
  for (kp = 0; kp < this.polyPieces.length; kp++) {
    for (kn = 0; kn < this.polyPieces[kp].pieces.length; kn++) {
      ...
      // Update the target position of the puzzle piece to be centered and scattered randomly
// Get half width and height
let vh = window.innerHeight * 0.01;
let vw = window.innerWidth * 0.01;
let halfVH = vh * 40;   // 50vh is middle of viewport height
let halfVW = vw * 20;   // 50vw is middle of viewport width
let scatterRange = 0.25;   // adjust this value to increase/decrease scatter range

let minX = halfVW - scatterRange * halfVW;
let maxX = halfVW + scatterRange * halfVW;
let minY = halfVH - scatterRange * halfVH;
let maxY = halfVH + scatterRange * halfVH;

pc.pTarget = new Point(minX + Math.random() * (maxX - minX), minY + Math.random() * (maxY - minY));
    }
  }
...
// won ?
  ...
  //window.top.location.href = 'puzzle-won.php'; // redirect to win page on the parent window
// YES ! tell the player
...
const imageList = [
  ...
  'chalkzone.jpg',
  'dexter.jpg',
  'dora_the_Explorer.webp',
  'jimmy.jpg',
  'naruto-poster-01.jpg',
  'oggy.jpg',
  'oggy_1.jpg',
  'ogg_love.jpg',
  'pokemon_ash.jpg',
...
window.addEventListener("load", function(){
  ...
  // Use the random image for your game
  let img = `assets/img/${randomImage}`;

  var image = document.getElementById("gt_game");
  image.src = img; 
  
  autoStart = isMiniature(); // used for a nice miniature in CodePen
  // get .puzzlearea width and height
  let x = new Puzzle({ 
    img: img,
